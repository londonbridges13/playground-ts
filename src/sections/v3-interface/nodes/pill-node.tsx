...
// ----------------------------------------------------------------------
// Pill Node Component - Capsule/Stadium shape
// ----------------------------------------------------------------------

export const PillNode = memo(({ data, isConnectable, selected, id }: NodeProps) => {
  const nodeIndex = (data.index as number) ?? 0;
  const isExiting = data.isExiting ?? false;

  // Dimensions
  const width = (data.width as number) ?? 160;
  const height = (data.height as number) ?? 60;

  // Background
  const backgroundImage = (data.backgroundImage as string) ?? null;
  const backgroundColor = (data.backgroundColor as string) ?? '#ec4899';
  ...
  // Pill shape uses height/2 for border radius to create stadium shape
        ...
        isExiting
          ...
          : {
              ...
              boxShadow: isConnectionTarget
                ? ['0 0 0 3px rgba(236, 72, 153, 0.6)', '0 0 0 8px rgba(236, 72, 153, 0.3)']
    ...
    >
      ...
      >
        ...
        <Handle
          ...
          style={{
            width: 10, height: 10, background: '#ec4899', border: '2px solid #fff',
